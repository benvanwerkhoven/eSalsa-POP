...
! !PUBLIC DATA MEMBERS:
  ...
  !-----------------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block,6,max_blocks_clinic), public ::   &
      SHF_SFLUX_TAVG

   integer (int_kind), public ::   &
      tavg_SWNET,       &! tavg id for net shortwave heat flux
      tavg_LWNET,       &! tavg id for net longwave heat flux
      tavg_LATENT,      &! tavg id for latent heat flux
      tavg_SENSIBLE,    &! tavg id for sensible heat flux
      tavg_T_WEAK_REST, &! tavg id for weak restoring heat flux
      tavg_T_STRONG_REST ! tavg id for strong restoring heat flux
...
!-----------------------------------------------------------------------
   ...
   case ('normal-year')
      ...
      shf_bndy_loc  (shf_data_qsw)     = field_loc_center
      shf_bndy_type (shf_data_qsw)     = field_type_scalar
      shf_data_names(shf_data_qlw)     = 'QLW'
      shf_bndy_loc  (shf_data_qlw)     = field_loc_center
      shf_bndy_type (shf_data_qlw)     = field_type_scalar
      shf_data_names(shf_data_windspd) = 'WINDSPD'
      shf_bndy_loc  (shf_data_windspd) = field_loc_center
      shf_bndy_type (shf_data_windspd) = field_type_scalar
   ...
   case ('normal-year+flux')
      lsw_absorb = .true.
      normal_year_forcing = .true.
      shf_data_num_fields = 7
      allocate(shf_data_names(shf_data_num_fields), &
               shf_bndy_loc  (shf_data_num_fields), &
               shf_bndy_type (shf_data_num_fields))
      shf_data_sst        = 1
      ...
      shf_bndy_loc  (shf_data_windspd) = field_loc_center
      shf_bndy_type (shf_data_windspd) = field_type_scalar
      shf_data_names(shf_data_flux)    = 'FLUX'
      shf_bndy_loc  (shf_data_flux)    = field_loc_center
      shf_bndy_type (shf_data_flux)    = field_type_scalar
   ...
   case ('monthly-equal','monthly-calendar')
      ...
      case ('normal-year')
         ...
         io_qsw     = construct_io_field(                        &
                      ...
                      field_type = shf_bndy_type(shf_data_qsw),  &
                      d3d_array=TEMP_DATA(:,:,:,:,shf_data_qsw))
         io_qlw     = construct_io_field(                           &
                      trim(shf_data_names(shf_data_qlw)),       &
                      i_dim, j_dim, month_dim,                      &
                      field_loc = shf_bndy_loc(shf_data_qlw),   &
                      field_type = shf_bndy_type(shf_data_qlw), &
                      d3d_array=TEMP_DATA(:,:,:,:,shf_data_qlw))
         io_windspd = construct_io_field(                           &
                      trim(shf_data_names(shf_data_windspd)),       &
                      i_dim, j_dim, month_dim,                      &
                      field_loc = shf_bndy_loc(shf_data_windspd),   &
         ...
         call data_set(forcing_file, 'define', io_qair)
         call data_set(forcing_file, 'define', io_qsw)
         call data_set(forcing_file, 'define', io_qlw)
         call data_set(forcing_file, 'define', io_windspd)
         ...
         call data_set(forcing_file, 'read', io_qair)
         call data_set(forcing_file, 'read', io_qsw)
         call data_set(forcing_file, 'read', io_qlw)
         call data_set(forcing_file, 'read', io_windspd)
         ...
         do n=1,12
            ...
            SHF_DATA (:,:,iblock,shf_data_qsw,n)     = &
            TEMP_DATA(:,:,n,iblock,shf_data_qsw)
            SHF_DATA (:,:,iblock,shf_data_qlw,n) = &
            TEMP_DATA(:,:,n,iblock,shf_data_qlw)
            SHF_DATA (:,:,iblock,shf_data_windspd,n) = &
            TEMP_DATA(:,:,n,iblock,shf_data_windspd)
         end do
         end do
...
!maltrud make sure ghost cells are ok
!        do i=1,shf_data_num_fields
!        do n=1,12
!           call update_ghost_cells(SHF_DATA(:,:,:,i,n), bndy_clinic, &
...
!        end do
      ...
      case ('normal-year+flux')
         ...
         io_flux    = construct_io_field(                           &
                      trim(shf_data_names(shf_data_flux)),       &
                      i_dim, j_dim, month_dim,                      &
                      field_loc = shf_bndy_loc(shf_data_flux),   &
                      field_type = shf_bndy_type(shf_data_flux), &
                      d3d_array=TEMP_DATA(:,:,:,:,shf_data_flux))
         ...
         call data_set(forcing_file, 'define', io_qlw)
         call data_set(forcing_file, 'define', io_windspd)
         call data_set(forcing_file, 'define', io_flux)
         ...
         call data_set(forcing_file, 'read', io_qlw)
         call data_set(forcing_file, 'read', io_windspd)
         call data_set(forcing_file, 'read', io_flux)
...
!-----------------------------------------------------------------------
    ...
    call define_tavg_field(tavg_SWNET,'SWNET',2,           &
                          long_name='net shortwave heat flux',           &
                          units='W/m^2', grid_loc='2111')

    call define_tavg_field(tavg_LWNET,'LWNET',2,           &
                          long_name='net longwave heat flux',           &
                          units='W/m^2', grid_loc='2111')

    call define_tavg_field(tavg_LATENT,'LATENT',2,           &
                          long_name='Latent heat flux',           &
                          units='W/m^2', grid_loc='2111')

    call define_tavg_field(tavg_SENSIBLE,'SENSIBLE',2,           &
                          long_name='Sensible heat flux',           &
                          units='W/m^2', grid_loc='2111')

    call define_tavg_field(tavg_T_WEAK_REST,'T_WEAK_REST',2,           &
                          long_name='Weak restoring heat flux',           &
                          units='W/m^2', grid_loc='2111')

    call define_tavg_field(tavg_T_STRONG_REST,'T_STRONG_REST',2,           &
                          long_name='Strong restoring heat flux',           &
                          units='W/m^2', grid_loc='2111')
...
!-----------------------------------------------------------------------
   ...
   case ('monthly-equal','monthly-calendar')
      ...
      case ('normal-year','normal-year+flux')
         call calc_shf_normal_year(STF,12)
...
!----------------------------------------------------------------------
      ...
      SHF_SFLUX_TAVG(:,:,1,iblock) = SHF_COMP(:,:,iblock,shf_comp_qsw)* &
                                     OCN_WGT(:,:,iblock)
      SHF_SFLUX_TAVG(:,:,2,iblock) = SHF_COMP(:,:,iblock,shf_comp_qlw)* &
                                     OCN_WGT(:,:,iblock)
      SHF_SFLUX_TAVG(:,:,3,iblock) = SHF_COMP(:,:,iblock,shf_comp_qlat)* &
                                     OCN_WGT(:,:,iblock)
      SHF_SFLUX_TAVG(:,:,4,iblock) = SHF_COMP(:,:,iblock,shf_comp_qsens)* &
                                     OCN_WGT(:,:,iblock)

      SHF_SFLUX_TAVG(:,:,5,iblock) = SHF_COMP(:,:,iblock,shf_comp_wrest)
      SHF_SFLUX_TAVG(:,:,6,iblock) = SHF_COMP(:,:,iblock,shf_comp_srest)
...
!***********************************************************************
!BOP
! !IROUTINE: calc_shf_normal_year
! !INTERFACE:

   subroutine calc_shf_normal_year(STF, time_dim)
...
!maltrud bug BUG
!     SHF_COMP(:,:,iblock,shf_comp_qsw) = sw_mod_albedo*sw_mod_fact* &
      SHF_COMP(:,:,iblock,shf_comp_qsw) = sw_mod_albedo* &
                                 SHF_DATA(:,:,iblock,shf_data_qsw,now)

      SHF_COMP(:,:,iblock,shf_comp_qlw) = &
                            SHF_DATA(:,:,iblock,shf_data_qlw,now)  &
                            -emissivity*stefan_boltzmann*  &
                            (TRACER(:,:,1,1,curtime,iblock) + T0_Kelvin)**4
...
!----------------------------------------------------------------------
!  if applied flux, replace strong restoring term with it and zero weak
!----------------------------------------------------------------------

      if (shf_formulation == 'normal-year+flux') then
         SHF_COMP(:,:,iblock,shf_comp_srest) = SHF_DATA(:,:,iblock,shf_data_flux,now)
         SHF_COMP(:,:,iblock,shf_comp_wrest) = c0
      endif
...
!EOC
 ...
 end subroutine calc_shf_normal_year
