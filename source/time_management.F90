...
!
! !REVISION HISTORY:
!  SVN:$Id: time_management.F90 17212 2009-07-20 23:01:42Z njn01 $
!
! !USES:
...
!-----------------------------------------------------------------------
   ...
   logical (log_kind) :: &
      robert_filter_standard,  &!  flag for standard Robert filter
      robert_filter_modified,  &!  flag for modified Robert filter
   ...
   integer (int_kind), parameter :: &
      ...
      tmix_avgfit  = 4,  &! use averaging step for time mixing, 
                          ...
                          !  a timestep
      tmix_robert1 = 5,  &! use standard Robert filter for time mixing
      tmix_robert2 = 6    ! use modified Robert filter for time mixing
   ...
   real (r8) :: &
      robert_filter_weight,     &! weighting value for filter correction
      robert_filter_alpha,      &! relative weight for curtime filter
      robert_curtime,           &! total weight for curtime filter
      robert_newtime,           &! total weight for newtime filter
...
!-----------------------------------------------------------------------
   ...
   robert_filter_weight   = 0.01_POP_r8
   robert_filter_alpha    = 0.53_POP_r8
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      ...
      case ('robert1')
         tmix_iopt = tmix_robert1
      case ('robert2')
         tmix_iopt = tmix_robert2
      case default
         tmix_iopt = -1000
      end select
   endif
...
!-----------------------------------------------------------------------
!  set Robert-Asselin Filter quantities if necessary
!-----------------------------------------------------------------------

   if (tmix_iopt == tmix_robert1) then
      robert_filter_standard = .true.
      robert_filter_modified = .false.
      robert_curtime = robert_filter_weight
      robert_newtime = c0
   endif
   if (tmix_iopt == tmix_robert2) then
      robert_filter_standard = .false.
      robert_filter_modified = .true.
      robert_curtime = robert_filter_weight*robert_filter_alpha
      robert_newtime = robert_filter_weight*(robert_filter_alpha - c1)
   endif
