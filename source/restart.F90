...
!
! !REVISION HISTORY:
!  SVN:$Id: restart.F90 17212 2009-07-20 23:01:42Z njn01 $
!
! !USES:
   ...
   use forcing_sfwf, only: sfwf_interp_last, sum_precip, precip_fact,       &
       ssh_initial, sal_initial, lfw_as_salt_flx
...
! !INTERFACE:
 ...
 subroutine write_restart(restart_type, errorCode)
...
!maltrud initialize cpl_write_restart to never
   call init_time_flag('cpl_write_restart',cpl_write_restart, default=.false.,  &
                        owner    = 'init_restart',               &
                        freq_opt = freq_opt_never,               &
                        freq     = restart_freq)
