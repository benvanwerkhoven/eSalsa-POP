...
!-----------------------------------------------------------------------
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'init_grid2: error calculating T coordinates')
      return
   endif
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'init_grid2: error updating ANGLET halo regions')
      return
   endif
...
!-----------------------------------------------------------------------
   ...
   if (lremove_points) then
      ...
      call remove_points(errorCode)

      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_grid2: error in remove points')
         return
      endif
   ...
   if (topo_smooth) then
      ...
      call smooth_topography(errorCode)

      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_grid2: error in smooth topography')
         return
      endif
   ...
   if (partial_bottom_cells) then
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_grid2: error updating DZU halo regions')
         return
      endif
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'init_grid2: error updating KMU halo regions')
      return
   endif
...
!-----------------------------------------------------------------------
   ...
   integer (POP_i4) :: &
      nu,                &! i/o unit number
      rec_length        ,&! record length for direct access i/o
      ioerr               ! i/o error flag
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      ...
      inquire (iolength = rec_length) DZBC_G
      open(nu, file=bottom_cell_file,status='old',form='unformatted', &
               access='direct', recl=rec_length, iostat=ioerr)
   endif
...
!-----------------------------------------------------------------------
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'remove points: error updating KMT halo regions')
      return
   endif
...
! !INTERFACE:
 ...
 subroutine smooth_topography(errorCode)
...
!-----------------------------------------------------------------------
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'smooth_topography: error updating KMT halo regions')
      return
   endif
...
!-----------------------------------------------------------------------
   ...
   integer (POP_i4) :: &
      k, n,              &! loop counters
      nu,                &! i/o unit number
      rec_length,        &! record length of file
      ioerr,             &! i/o error flag
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      ...
      inquire (iolength = rec_length) REGION_G
...
!-----------------------------------------------------------------------
   ...
   call POP_HaloUpdate(TLAT, POP_haloClinic, POP_gridHorzLocCenter, &
                             POP_fieldKindScalar, errorCode,        &
                             fillValue = POP_undefinedR8)
                           
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'calc_tpoints: error updating TLAT halo regions')
      return
   endif
   ...
   if (errorCode /= POP_Success) then
      call POP_ErrorSet(errorCode, &
         'calc_tpoints: error updating TLON halo regions')
      return
   endif
...
!-----------------------------------------------------------------------
     ...
     if (errorCode /= POP_Success) then
        call POP_ErrorSet(errorCode, &
           'fill_points: error updating smoothed topo halo regions')
        return
     endif
